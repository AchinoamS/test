(function () {
    ...
    picker = angular.module('daterangepicker', ['pascalprecht.translate']);

    picker.constant('dateRangePickerConfig', {
        clearLabel: 'Clear',
        applyClass: 'btn-darkgray',
        locale: {
            separator: ' - ',
            format: 'YYYY/MM/D',
            applyLabel: 'Apply',
    ...
    picker.directive('dateRangePicker', ['$compile', '$timeout', '$parse', 'dateRangePickerConfig', '$rootScope', '$translate', function ($compile, $timeout, $parse, dateRangePickerConfig, $rootScope, $translate) {
        return {
            require: 'ngModel',
            restrict: 'A',
            scope: {
            ...
            link: function ($scope, element, attrs, modelCtrl) {
                var customOpts, el, opts, _clear, _format, _init, _initBoundaryField, _mergeOpts, _fixOpts, _picker, _setDatePoint, _setEndDate, _setStartDate, _setViewValue, _validate, _validateMax, _validateMin;
                _mergeOpts = function () {
                ...
                _fixOpts = function (_opts) {
                    if (_opts && _opts._opens) {
                        if (_opts._opens == "near") {
                            _opts.opens = $('body').css('direction') == "rtl" ? "right" : "left";
                        } else if (_opts._opens == "far") {
                            _opts.opens = $('body').css('direction') == "rtl" ? "left" : "right";
                        }
                    }
                ...
                _init = function () {
                    ...
                    _opts.locale.applyLabel = $translate.instant('Apply');
                    _opts.locale.cancelLabel = $translate.instant('Cancel');
                    _opts.locale.customRangeLabel = $translate.instant('Custom Range');

                    var ranges = {};
                    $.each(_opts.ranges, function (key, val) {
                        ranges[$translate.instant(key)] = val;
                    });
                    _opts.ranges = ranges;

                    el.daterangepicker(angular.extend(_opts, {
                ...
                $rootScope.$on("appSettingsChanged", function (o, args) {
                    $timeout(_init);
                });
